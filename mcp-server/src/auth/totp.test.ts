...
describe("buildOtpAuthUri", () => {
  it("includes the required Key URI Format fields", () => {
    const uri = buildOtpAuthUri("JBSWY3DPEHPK3PXP", "quincy", "CoreOps");
    expect(uri).toMatch(/^otpauth:\/\/totp\//);
    expect(uri).toContain("CoreOps:quincy");
    expect(uri).toContain("secret=JBSWY3DPEHPK3PXP");
    expect(uri).toContain("issuer=CoreOps");
  ...
  it("URL-encodes an issuer/account containing special characters", () => {
    const uri = buildOtpAuthUri("JBSWY3DPEHPK3PXP", "a b", "Core Ops");
    expect(uri).toContain("Core%20Ops:a%20b");
  });
});
